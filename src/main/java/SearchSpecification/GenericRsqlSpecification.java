    ...
    public Predicate toPredicate(final Root<T> root, final CriteriaQuery<?> query, final CriteriaBuilder builder) {
       ...
       //  Object argument = args.get(0);
        ...
        case EQUAL: {
          ...
          //  return	builder.equal(root.<String> get(property), argument.toString());
              
            	
            } else if (argument == null) {
                return builder.isNull(root.get(property));
            } else {
                return builder.equal(root.<String> get(property),argument );
            }
        }
        case NOT_EQUAL: {
            if (argument instanceof String) {
 ...
 // This method will help us diving deep into nested property using the dot convention
    ...
    private List<Object> castArguments(Path<?> propertyExpression) {
        ...
        return arguments.stream().map(arg -> {
            ...
            else if (type.equals(sub.class)) return sub.valueOf(arg);
            else return arg;
        }).collect(Collectors.toList());
    }
    */
